...
if display ~=0
    figure(1)
    imagesc([0.5 numGridsX-0.5],[0.5 numGridsY-0.5],flipud(myMat), 'AlphaData', .5);
%     imagesc(flipud(myMat), 'AlphaData', .1)                         % Draw grids & plot lines
    set(gca,'YDir','normal')
    hold on
   %if display==2, don't plot grids
    if display ~=2
        %plot grids
        for k = 1:numGridsX-1
    ...
    %plot([hvix(1,1) hvix(end,1)], [hvix(1,2) hvix(end,2)],'r','LineWidth',2) % Plot line
    scatter(hvix(:,1),hvix(:,2),'MarkerFaceColor','w')
    grid on
    hold off
    axis equal
