...
%ver2: also invert for reflection coeffecient
    ...
    %ver2_4: just use the TV prior because it works the best, but we need
    %layer information - include it
...
for n=1:length(txLoc)
    rayGeometryBed(:,:,n)=pathLenMasks_ver5(txLoc(n),rxLoc(n),wGrid,hGrid,numGridsY,numGridsX,0);
end
for n=1:length(txLoc)
    now=pathLenMasks_ver5(txLoc(n),rxLoc(n),wGrid,hGrid/2,numGridsY/2,numGridsX,0);
    rayGeometryLayer(:,:,n)=[now;zeros(10,40)];
end

rayGeometry(:,:,1:length(txLoc))=rayGeometryBed;
rayGeometry(:,:,length(txLoc)+1:2*length(txLoc))=rayGeometryLayer;
% 
% %Display masks (ray geometry)
% gcf=figure()
% for k=1:size(rayGeometry,3)
%     imagesc(rayGeometry(:,:,k))
%     title('Geometry')
...
reflMask(:,:,length(txLoc)+1:2*length(txLoc))=zeros(gridResolution(1),gridResolution(2),size(rayGeometryLayer,3));
